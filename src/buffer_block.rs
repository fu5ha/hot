...
impl BufferBlockPool {
    ...
    /// Allocate a new BufferBlock from the pool. Will not attempt to reuse a previously allocated recycled buffer.
    ///
    /// # Parameters
    ///
    ...
    pub fn recycle_block(&mut self, device: &mut Device, block: BufferBlock) -> Result<(), BlockRecycleError> {
        ...
        for (_, owned_buffer) in owned_block.allocated_buffers.drain() {
